...
const $63d5d05912b88f0d$var$fpv = ({ camera: camera, THREE: THREE, domElement: domElement, controls: controls })=>{
    const property = {
        isMoving: false,
        height: 1.6,
        speed: 5,
    ...
    function fpvArrowDownFunction(e) {
        if (property.trigger.arrow) switch(e.key){
            case "ArrowUp":
    ...
    function fpvArrowUpFunction(e) {
        if (property.trigger.arrow) {
            ...
            if (!property.pressed.ArrowUp && !property.pressed.ArrowDown && !property.pressed.ArrowLeft && !property.pressed.ArrowRight) property.isMoving = false;
        }
    }
    function fpvWASDDownFunction(e) {
        if (property.trigger.wasd) switch(e.key){
            case "w":
    ...
    function fpvWASDUpFunction(e) {
        if (property.trigger.wasd) {
            ...
            if (!property.pressed.KeyW && !property.pressed.KeyA && !property.pressed.KeyS && !property.pressed.KeyD) property.isMoving = false;
        }
    }
    return {
        height: {
        ...
        update: (delta)=>{
            if (property.isMoving) {
                ...
                if (property.pressed.ArrowDown || property.pressed.KeyS) pm = -1;
                ...
                if (property.pressed.ArrowLeft || property.pressed.KeyA) lr = 1;
            ...
            controls.target.set(property.position.x + Math.cos(property.viewAngle.theta) * Math.cos(property.viewAngle.phi), property.height + Math.sin(property.viewAngle.theta), property.position.z + Math.cos(property.viewAngle.theta) * Math.sin(property.viewAngle.phi));
        },
        connect: ({ mouse: mouse = true, mouseDownMove: mouseDownMove = false, arrow: arrow = true, wasd: wasd = true, touch: touch = true, height: height = null, speed: speed = null, viewSpeed: viewSpeed = null, position: position = null } = {})=>{
            ...
            if (touch) {
                ...
                domElement.addEventListener("touchmove", fpvTouchMoveFunction, {
                    passive: false
                });
            }
        },
...
const $30c5471b9d51ed43$var$instances = ({ scene: scene, THREE: THREE })=>{
    return (originalMesh, count = 1, { position: position = [
        0,
        0,
        0
    ], rotation: rotation = [
        ...
        0,
        0
    ], removeOriginal: removeOriginal = true, offset: offset = [
        0,
        0,
        0
    ], layout: layout = "line", radius: radius = 1, castShadow: castShadow = null, receiveShadow: receiveShadow = null, autoAdd: autoAdd = true } = {})=>{
        const m = new THREE.InstancedMesh(originalMesh.geometry, originalMesh.material, count);
        ...
        for(let i = 0; i < count; i++){
            ...
            switch(layout){
                case "none":
                    break;
                case "line":
                    dummy.position.set(originalMesh.position.x + offset[0] * i, originalMesh.position.y + offset[1] * i, originalMesh.position.z + offset[2] * i);
                    break;
                case "grid":
                case "grid-xy":
                case "grid-xz":
                ...
                case "grid-zy":
                    ...
                    dummy.position.set(originalMesh.position.x + (axes[0] === "x" ? i % columns * offset[0] : axes[1] === "x" ? Math.floor(i / columns) * offset[0] : 0), originalMesh.position.y + (axes[0] === "y" ? i % columns * offset[1] : axes[1] === "y" ? Math.floor(i / columns) * offset[1] : 0), originalMesh.position.z + (axes[0] === "z" ? i % columns * offset[2] : axes[1] === "z" ? Math.floor(i / columns) * offset[2] : 0));
                    break;
                case "cube":
                case "cube-xy":
                case "cube-xz":
                ...
                case "cube-zy":
                    ...
                    dummy.position.set(originalMesh.position.x + (axes[0] === "x" ? i % cubeColumns * offset[0] : axes[1] === "x" ? Math.floor(i / cubeColumns) % cubeColumns * offset[0] : Math.floor(i / (cubeColumns * cubeColumns)) * offset[0]), originalMesh.position.y + (axes[0] === "y" ? i % cubeColumns * offset[1] : axes[1] === "y" ? Math.floor(i / cubeColumns) % cubeColumns * offset[1] : Math.floor(i / (cubeColumns * cubeColumns)) * offset[1]), originalMesh.position.z + (axes[0] === "z" ? i % cubeColumns * offset[2] : axes[1] === "z" ? Math.floor(i / cubeColumns) % cubeColumns * offset[2] : Math.floor(i / (cubeColumns * cubeColumns)) * offset[2]));
                    break;
                case "circle":
                case "circle-xy":
                case "circle-xz":
                ...
                case "circle-zx":
                case "circle-zy":
                    dummy.position.set(originalMesh.position.x + (axes[0] === "x" ? Math.cos(theta) * radius : axes[1] === "x" ? Math.sin(theta) * radius : 0), originalMesh.position.y + (axes[0] === "y" ? Math.cos(theta) * radius : axes[1] === "y" ? Math.sin(theta) * radius : 0), originalMesh.position.z + (axes[0] === "z" ? Math.cos(theta) * radius : axes[1] === "z" ? Math.sin(theta) * radius : 0));
                    break;
            }
        ...
        m.setColor = (color)=>{
            ...
            for(let i = 0; i < count; i++)m.setColorAt(i, colorObj);
...
const $aad201fb457a6818$var$vrm = ({ scene: scene, load: load })=>{
    return async (url, { position: position = [
        0,
        0,
        0
...
const $9369bcd5c26d214d$var$bvh = ({ THREE: THREE })=>{
    return async (url, vrm, { effectiveWeight: effectiveWeight = 1, timeScale: timeScale = 1000, onProgress: onProgress = (p)=>{}, nameList: nameList = [
        "head",
        "neck",
        "chest",
        ...
        function createClip(vrm, bvh) {
            ...
            for(let i = 0; i < idList.length; i++){
                ...
                tracks.push(track);
                if (keys[0].pos) {
                    const posValues = keys.flatMap((k)=>k.pos ?? [
                            0,
                            0,
                            0
                        ]);
                    const posTrack = new THREE.VectorKeyframeTrack(`${bone.name}.position`, times, posValues);
...
var $9369bcd5c26d214d$export$2e2bcd8739ae039 = $9369bcd5c26d214d$var$bvh;
...
const $bf21102bcb721113$var$use = [
    ...
    {
        name: "bvh2",
        fn: (0, $9369bcd5c26d214d$export$2e2bcd8739ae039)
    }
];
